...
foreach ($csv_array as $index => $row) {
	if (in_array($row["IDENTIFIER"],$active_atlases)) {
		...
		if (count($active_atlases)>1) {
			...
			$display_list[$row["IDENTIFIER"]]['header'] .= "\t\t\t\t\t\t\t<span class=\"arrow arrow-d\"></span>\n";
		}
		...
		if (count($active_atlases)>1) {
			...
			$display_list[$row["IDENTIFIER"]]['header'] .= "\t\t\t\t\t\t<div id=\"".$row["IDENTIFIER"]."CurrentContent\" style=\"display:block;\">\n";
		}
